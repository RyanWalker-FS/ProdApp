...
class CollaborativeService {
  ...
  private setupSocketListeners(): void {
    ...
    socketService.on("field_unlocked", (data: {
      field: string;
      userId: string;
    }) => {
      // Update UI to show field is available
      // Note: This is simplified - in a real app you'd track all locks
  ...
  onStateChange(
    callback: (state: CollaborativeState) => void
  ): () => void {
  ...
  onLocksChange(callback: (locks: EditLock[]) => void): () => void {
    ...
    return () => {
      this.lockListeners = this.lockListeners.filter(
        (cb) => cb !== callback
      );
    };
  }
